...
searchByInputBtn.addEventListener("click", async () => {
	if (searchInput.value) {
		try {
			const link = await apiLinkByCity(searchInput.value);
...
document.addEventListener("DOMContentLoaded", async () => {
	...
	if (APILink) {
		deleteNodes();
		const data = await getWeatherData(APILink);
		weatherData = data;
		loadWeatherDataOnPage(weatherData);
	} else {
		forecastContainer.style.display = "none";
...
const populate24HourWeather = async (data) => {
    ...
    */
	const mySwiper = new Swiper(".h-24__swiper", {
...
const populateWeeklyWeather = async (data) => {
    ...
    что созданный скелет свайпера
    */
	const myySwiper = new Swiper(".weekly__swiper", {
		modules: [Navigation, Keyboard],
		observer: true,
		observeParents: true,
		observeSlideChildren: true,
